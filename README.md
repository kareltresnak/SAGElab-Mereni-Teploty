...
#### 1) Stáhněte si zdrojové kódy pomocí GIT CLONE

```text
# Pokud nemáte git a Node.js s npm:
https://git-scm.com/install/

https://nodejs.org/en/download
```
```bash
# Poté spusťte tento příkaz ve složce, do které je chcete uložit
git clone https://github.com/kareltresnak/SAGElab-Mereni-Teploty.git

# Přejděte do klonované složky
cd SAGElab-Mereni-Teploty
...
# Instalace vývojových závislostí (Kompilátor a typy)
npm install --save-dev typescript @types/node @types/express @types/sqlite3 @types/cors
```

#### 2) Ve složce SAGElab-Mereni-Teploty vytvořte soubor `tsconfig.json` a vložte v něj následující kód
...
#### 3) Otevřte soubor `package.json` a upravte sekci `scripts` takto:
...
#### 4) Proveďte sestavení a spusťte server

```bash
npm run build
# Poté vždy stačí spustit tímto příkazem
npm start
```
...
# Při případné instalaci Pythonu zaškrněte políčko `pip`
